...
for(i in 2:length(tc)) {
  acts[i,]$id<-i
  acts[i,]$type<-tc[i]
  if(tc[i]=="home" && !is.null(home_sa1)) { # re-use home SA1 and coords
    acts[i,]$sa1<-home_sa1 
    acts[i,]$x<-home_xy[1]
    acts[i,]$y<-home_xy[2]
    ...
    if (is.null(mode) || tc[i-1]=="home") { # mode can change from home only
      df<-findLocation(acts[i-1,]$sa1,acts[i,]$type)
    } else {
      df<-findLocationKnownMode(acts[i-1,]$sa1, acts[i,]$type, mode)
    }
    # save the SA1 and associated coordinates for this activity
    acts[i,]$sa1<-df[2]
